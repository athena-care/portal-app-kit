...
import type { default as WaAccordionItem } from "./_bundle_/src/components/accordion-item/accordion-item.js";
...
import type { WaTreeItem, default as treeItemContext } from "./_bundle_/src/components/tree-item/tree-item.js";
...
import type { default as WaAccordion } from "./_bundle_/src/components/accordion/accordion.js";
...
import type { default as WaCheckboxGroup } from "./_bundle_/src/components/checkbox-group/checkbox-group.js";
...
import type { default as WaKnownDate } from "./_bundle_/src/components/known-date/known-date.js";
...
import type { default as WaTimeInput } from "./_bundle_/src/components/time-input/time-input.js";
...
import type { default as WaDatePicker } from "./_bundle_/src/components/date-picker/date-picker.js";
import type { default as WaDateInput } from "./_bundle_/src/components/date-input/date-input.js";
...
import type { default as WaVideo } from "./_bundle_/src/components/video/video.js";
import type { default as WaVideoPlaylist } from "./_bundle_/src/components/video-playlist/video-playlist.js";
 ...
 * - `value`: The selected date(s). For `mode="single"`, an ISO date string (`YYYY-MM-DD`) or empty. For `mode="range"`, two ISO
 * dates separated by `/` (`YYYY-MM-DD/YYYY-MM-DD`). The property setter also accepts `Date` objects and
 * `{ from, to }` objects for ranges. 
 * - `min`: The earliest selectable date as `YYYY-MM-DD`. 
 * - `max`: The latest selectable date as `YYYY-MM-DD`. 
 * - `today`: Overrides the date considered "today". 
 * - `focused-date`/`focusedDate`: The currently focused date as `YYYY-MM-DD`. Drives roving tabindex and the visible month. 
 * - `view`: The current view. 
 * - `months`: Number of months rendered side-by-side. Either `1` or `2`. Set to `2` to see both ends of a range at once. 
 * - `page-by`/`pageBy`: Whether prev/next advances by the visible range (`months`) or one month at a time (`single`). 
 * - `first-day-of-week`/`firstDayOfWeek`: The first day of the week. The default `auto` uses the current locale's week info. To set a specific day, pass a
 * three-letter weekday name: `sun`, `mon`, `tue`, `wed`, `thu`, `fri`, or `sat`. 
 * - `with-outside-days`/`withOutsideDays`: Shows leading and trailing days from adjacent months. 
 * - `with-week-numbers`/`withWeekNumbers`: Shows an ISO week-number column. 
 * - `weekday-format`/`weekdayFormat`: The weekday header format. 
 * - `disabled`: Disables the entire picker. 
 * - `readonly`: Displays the current value without allowing changes. Cells remain focusable. 
 * - `disabled-dates`/`disabledDates`: A list of whitespace-separated ISO dates that should be disabled. The property accepts an array. 
 * - `disabled-days-of-week`/`disabledDaysOfWeek`: Weekdays to disable. Accepts a space-separated list of three-letter weekday names: `sun`, `mon`, `tue`, `wed`,
 * `thu`, `fri`, `sat` 
 * - `disable-past`/`disablePast`: Disable all dates strictly before `today`. 
 * - `disable-future`/`disableFuture`: Disable all dates strictly after `today`. 
 * - `min-range`/`minRange`: Minimum range length in days (`mode="range"` only). `0` disables the check. 
 * - `max-range`/`maxRange`: Maximum range length in days (`mode="range"` only). `0` disables the check. 
 * - `size`: Visual size. 
 * - `locale`: BCP-47 locale override. When empty, the inherited `lang` attribute is used. 
 * - `dir`: undefined 
 * - `lang`: undefined 
 * - `did-ssr`/`didSSR`: undefined 
 * - `isDateDisabled`: Author-supplied predicate that returns `true` when a date should be disabled. Runs in addition to declarative
 * `min` / `max` / `disabled-*` rules. JavaScript-only — set via property, not attribute. (property only)
 * - `dayContent`: Author-supplied function that returns custom content for a day cell. Receives a `Date` and returns a string of
 * HTML, a Lit `TemplateResult`, or `null` to use the default day number. Runs for every rendered day cell (including
 * outside days). A `day-YYYY-MM-DD` slot, when provided for the same date, wins over this function. Property only. (property only)
 * - `valueAsDate`: Read-only convenience getter: returns the selected date in `mode="single"`. (property only) (readonly)
 * - `valueAsRange`: Read-only convenience getter: returns the selected range in `mode="range"`. (property only) (readonly)
 * - `initialReflectedProperties`: undefined (property only)
 * - `internals`: undefined (property only)
 * 
 * #### Events
 ...
 * 
 * - `handleSizeChange() => void`: undefined
 * - `handleModeChange(_old?: WaDatePickerMode, next?: WaDatePickerMode) => void`: undefined
 * - `focus(options?: FocusOptions) => void`: Focuses the calendar at the currently focused day.
 * - `goToDate(date: string | Date) => void`: Scrolls the view to show the given date and sets the focused day.
 * - `goToToday() => void`: Equivalent to `goToDate(today)`.
 * - `clear() => void`: Clears the current selection and emits `input` then `change`.
 * 
 * #### CSS Parts
 * 
 * Custom selectors for styling elements within the component.
 ...
 * Component attributes and properties that can be applied to the element or by using JavaScript.
 * 
 * - `name`: The date input's name, submitted as a name/value pair with form data. 
 * - `value`/`defaultValue`: The default value of the form control. Used for form reset. 
 * - `disabled`: Disables the date input. 
 * - `required`: Makes the date input required for form submission. 
 * - `readonly`: Makes the input non-editable. The popup still opens for browsing. 
 * - `size`: The date input's size. 
 * - `appearance`: The date input's visual appearance. 
 * - `pill`: Draws a pill-style date input with rounded edges. 
 * - `label`: The date input's label. If you need to display HTML, use the `label` slot instead. 
 * - `hint`: The date input's hint. If you need to display HTML, use the `hint` slot instead. 
 * - `autocomplete`: Forwarded to the hidden form input (e.g., `'bday'`, `'cc-exp'`) to enable browser autofill. 
 * - `with-clear`/`withClear`: Shows a clear button when the date input has a value. 
 * - `with-label`/`withLabel`: Only required for SSR. Set to `true` if you're slotting in a `label` element. 
 * - `with-hint`/`withHint`: Only required for SSR. Set to `true` if you're slotting in a `hint` element. 
 * - `mode`: Selection mode. 
 * - `min`: Earliest selectable date as `YYYY-MM-DD`. Out-of-range dates are disabled in the popup calendar and a
 * committed value before `min` fails constraint validation with `rangeUnderflow`. 
 * - `max`: Latest selectable date as `YYYY-MM-DD`. Out-of-range dates are disabled in the popup calendar and a
 * committed value after `max` fails constraint validation with `rangeOverflow`. 
 * - `today`: Override "today" as `YYYY-MM-DD` (defaults to the runtime date). 
 * - `first-day-of-week`/`firstDayOfWeek`: The first day of the week in the popup calendar. 
 * - `disabled-dates`/`disabledDates`: Dates that cannot be selected. 
 * - `disabled-days-of-week`/`disabledDaysOfWeek`: Days of the week that cannot be selected. Accepts a space-separated list of three-letter weekday names. 
 * - `disable-past`/`disablePast`: Disable all dates strictly before today. 
 * - `disable-future`/`disableFuture`: Disable all dates strictly after today. 
 * - `min-range`/`minRange`: Minimum range length in days (range mode only). `0` disables. 
 * - `max-range`/`maxRange`: Maximum range length in days (range mode only). `0` disables. 
 * - `months`: Number of months rendered in the popup calendar. 
 * - `page-by`/`pageBy`: Whether prev/next pages by the visible range or one month at a time. 
 * - `with-outside-days`/`withOutsideDays`: Show leading/trailing days from adjacent months in the popup calendar. 
 * - `with-week-numbers`/`withWeekNumbers`: Show ISO 8601 week numbers in the popup calendar. 
 * - `weekday-format`/`weekdayFormat`: Weekday header format in the popup calendar. 
 * - `open`: Whether the popup calendar is open. 
 * - `placement`: Preferred popup placement. 
 * - `distance`: Distance in pixels between the popup and the input. 
 * - `custom-error`/`customError`: undefined 
 * - `dir`: undefined 
 * - `lang`: undefined 
 * - `did-ssr`/`didSSR`: undefined 
 * - `assumeInteractionOn`: Native `input` events do not fire on `role=spinbutton` elements (they aren't real `<input>`s). The component
 * dispatches a composed host `input` event on every segment edit, every step, and on calendar selection, so a
 * single `input` is enough to mark the field as interacted with. (property only)
 * - `popupId`: undefined (property only) (readonly)
 * - `keyboardHelpId`: undefined (property only) (readonly)
 * - `popup`: undefined (property only)
 * - `valueInput`: undefined (property only)
 * - `inputGroup`: undefined (property only)
 * - `calendar`: undefined (property only)
 * - `validationTarget`: Override this to change where constraint validation popups are anchored. (property only) (readonly)
 * - `value`: The date input's value. ISO 8601 `YYYY-MM-DD` for single mode, `YYYY-MM-DD/YYYY-MM-DD` for range mode (with
 * `from <= to`). The setter also accepts a `Date` or a range object with `from` and `to` properties. (property only)
 * - `isDateDisabled`: JS-only callback for custom date disabling. Forwarded to the popup calendar. (property only)
 * - `dayContent`: JS-only callback for custom day-cell content. Forwarded to the popup calendar. (property only)
 * - `valueAsDate`: The selected date as a `Date` (single mode only). (property only) (readonly)
 * - `valueAsRange`: The selected range as an object with `from` and `to` properties (range mode only). (property only) (readonly)
 * - `input`: undefined (property only)
 * - `valueHasChanged`: undefined (property only)
 * - `hasInteracted`: undefined (property only)
 * - `states`: undefined (property only)
 ...
 * - `handleOpenChange() => void`: undefined
 * - `focus(options?: FocusOptions) => void`: Sets focus on the first empty (else first) segment.
 * - `blur() => void`: Removes focus from the date input.
 * - `show() => Promise<void>`: Opens the popup calendar.
 * - `hide() => Promise<void>`: Closes the popup calendar.
 * - `clear() => void`: Clears the current value and emits `wa-clear`, `input`, and `change`. Mirrors activating the clear button. No-op
 ...
 * - `capture`: On mobile devices, specifies which camera or microphone to use for capturing media. Use `user` for the front-facing
 * camera/microphone or `environment` for the rear-facing one. This attribute is only used when `accept` includes an
 * image, video, or audio type and may be ignored on devices that lack the corresponding hardware. 
 * - `with-label`/`withLabel`: Only required for SSR. Set to `true` if you're slotting in a `label` element so the server-rendered markup
 * includes the label before the component hydrates on the client. 
 ...
 * Component attributes and properties that can be applied to the element or by using JavaScript.
 * 
 * - `controls`: The video's controls preset.
 * - `none` — no controls are shown.
 * - `standard` — shows the timeline, play/pause, volume, captions, and fullscreen.
 * - `full` — all of the above plus playback speed and picture-in-picture. 
 * - `thumbnails`: A URL pointing to a WebVTT file for timeline thumbnail previews. 
 * - `src`: The URL of the video source. For multiple formats, use `<source>` elements instead. 
 * - `poster`: Poster image URL 
 * - `title`: The video's title. 
 * - `playing`: Indicates whether the video is currently playing. 
 * - `muted`: When set, the video will be muted. 
 * - `volume`: The video's volume. 
 * - `duration`: The total duration of the video in seconds. 
 * - `currentTime`: The current playback position in seconds. 
 * - `autoplay`: Enables autoplay when the component connects. 
 * - `loop`: Loops the video when playback ends. 
 * - `autoplay-muted`/`autoplayMuted`: Enables autoplay in a muted state. 
 * - `autoplay-on-visible`/`autoplayOnVisible`: Automatically resumes playback when the player scrolls back into view after being paused by scrolling out. 
 * - `preload`: Controls how the browser preloads the video. Defaults to 'metadata' to minimize data usage. 
 * - `icon-library`/`iconLibrary`: Icon library used for all built-in control icons. Defaults to 'system'. 
 * - `dir`: undefined 
 * - `lang`: undefined 
 * - `did-ssr`/`didSSR`: undefined 
 * - `initialReflectedProperties`: undefined (property only)
 ...
 * - `togglePlay() => void`: Toggles between play and pause.
 * - `toggleMute() => void`: Toggles the muted state.
 * - `seek(time: number) => void`: Seeks to a specific time in the video.
 * - `setVolume(volume: number) => void`: Sets the volume level.
 * - `setPlaybackRate(rate: number) => void`: Sets the playback rate (speed).
 * - `requestFullscreen() => Promise<void>`: Enters fullscreen mode.
 * - `exitFullscreen() => Promise<void>`: Exits fullscreen mode.
 * - `getVideoElement() => HTMLVideoElement | null`: Gets the native video element.
 * - `getState() => {
 *     playing: boolean;
 *     currentTime: number;
 *     duration: number;
 ...
 * CSS variables available for styling the component.
 * 
 * - `--controls-color`: The text and icon color used throughout the controls overlay, title overlay, and mobile controls. (default: `white`)
 * - `--controls-background`: The background of the controls bar and mobile controls. (default: `var(--wa-color-surface-default)`)
 * - `--poster-play-button-background`: The background of the play button shown over the poster image. Also used to derive the hover state via color-mix(). (default: `var(--wa-color-surface-default)`)
 * 
 * #### CSS Parts
 * 
 * Custom selectors for styling elements within the component.
 ...
 * Component attributes and properties that can be applied to the element or by using JavaScript.
 * 
 * - `controls`: The controls preset forwarded to each child `<wa-video>`. 
 * - `icon-library`/`iconLibrary`: Icon library used for placeholder icons. 
 * - `dir`: undefined 
 * - `lang`: undefined 
 * - `did-ssr`/`didSSR`: undefined 
 * - `initialReflectedProperties`: undefined (property only)
 ...
 * - `goTo(index: number) => void`: Jumps to the video at the given index.
 * 
 * #### CSS Parts
 * 
 * Custom selectors for styling elements within the component.
