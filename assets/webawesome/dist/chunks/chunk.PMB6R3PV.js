...
var WaVideo = class extends WebAwesomeElement {
  constructor() {
    ...
    this.handleKeyDown = (e) => {
      ...
      if (handled.includes(e.key) || e.key >= "0" && e.key <= "9") {
        e.preventDefault();
      }
      switch (e.key) {
        case " ":
  ...
  handleDoubleTap(e) {
    ...
    const isLeft = e.clientX < wrapperRect.left + wrapperRect.width / 2;
  ...
  renderCaptionsDropdown() {
    if (!this.availableCaptions.length) return nothing;
    return html`
      <wa-dropdown
        placement="top"
        distance="8"
        @wa-select=${(e) => this.handleCaptionSelect(parseInt(e.detail.item.value, 10))}
      >
        <wa-button
          id="captions-button"
          slot="trigger"
  ...
  renderDesktopControls() {
    ...
    return html`
      ...
      this.formatTime(this.duration)
                      ...
                      ></wa-slider>
                      ${this.showThumbnail && this.currentThumbnail ? html`
                            <div
            ...
            <div>
              ${""}
              ${this.supportsVolumeControl && (!this.didSSR || this.didSSR && this.hasUpdated) ? html`
                    <wa-popover
                      for="volume-button"
                      placement="top"
                      distance="8"
              ...
              ${this.isCompact ? this.controls === "full" ? this.renderCompactOverflowMenu() : html`
                      <wa-button
                    ...
                    ` : html`
                    ${this.controls === "full" ? html`
                          <wa-dropdown
                            placement="top"
                            distance="8"
                            @wa-select=${(e) => this.handleSpeedSelect(parseFloat(e.detail.item.value))}
                          >
                            <wa-button
                              id="speed-button"
                              slot="trigger"
  ...
  renderControls() {
    ...
    return this.isMobile ? nothing : this.renderDesktopControls();
  }
  render() {
    return html`
        ...
        <div class="video-content">
          ...
          >
            <video
              ...
              poster=${this.currentPoster || nothing}
      ...
      (track) => html`
            ...
            ${this.isMobile ? html`<div class="video-tap-overlay" @click=${this.handleVideoClick}></div>` : nothing}
            ${this.showPoster && this.currentPoster ? html`
                  <div
                ...
                ` : nothing}
            ${this.currentTitle ? html` <div class="video-title-overlay" part="video-title-overlay">${this.currentTitle}</div> ` : nothing}
            ${this.currentCaption ? html`
          ...
          ${this.controls !== "none" && this.isMobile ? this.renderMobileControls() : nothing}
        </div>
...
__decorateClass([
  property({ type: Boolean, attribute: "autoplay-muted" })
], WaVideo.prototype, "autoplayMuted", 2);
__decorateClass([
  property({ type: Boolean, attribute: "autoplay-on-visible" })
