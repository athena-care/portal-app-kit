...
import {
  ...
  segmented_field_styles_default
} from "./chunk.36KCQ52O.js";
import {
  formatIsoDate,
  parseIsoDate,
  parseRange
...
} from "./chunk.WWLUKDGE.js";
import {
  WebAwesomeFormAssociatedElement
} from "./chunk.WPF5VWVD.js";
import {
  warnDeprecatedSize
...
var WaDateInput = class extends WebAwesomeFormAssociatedElement {
  ...
  syncSegmentsFromCanonical() {
    this.segmentsController.clearBuffers();
    if (this.mode === "range") {
      const r2 = rangeIsoToSegments(this._value);
      this.fromSegments = r2.from;
      this.toSegments = r2.to;
      this.segments = { year: null, month: null, day: null };
    } else {
      ...
      "Use arrow keys to change values; press Alt+Down Arrow to open the calendar."
            ...
            >
              ...
              >
                ...
                ${this.forwardedDaySlots.map((name) => x`<slot name=${name} slot=${name}></slot>`)}
              </wa-date-picker>
            </div>
          </wa-popup>
        </div>
  ...
  renderRangeSegments(layout) {
    return [
      ...
      x`<span part="range-separator segment-literal" class="segment-literal range-separator" aria-hidden="true"
        >&nbsp;–&nbsp;</span
      >`,
  ...
  renderLiteral(text) {
    return x`<span part="segment-literal" class="segment-literal" aria-hidden="true">${text}</span>`;
  }
...
WaDateInput.css = [size_styles_default, form_control_styles_default, segmented_field_styles_default, date_input_styles_default];
