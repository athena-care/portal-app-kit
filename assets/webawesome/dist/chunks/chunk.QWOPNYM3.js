...
// _bundle_/src/components/data-grid/virtual-controller.ts
var RESIZE_SETTLE_MS = 150;
var VirtualController = class {
  constructor(host) {
    ...
    this.extractRange = (range) => {
      ...
      if (pinned != null && pinned >= 0 && pinned < this.count && !indexes.includes(pinned)) {
     ...
     */
    this.observeRectDebounced = (instance, cb) => {
      let latestRect = null;
      let forwardedRect = null;
      let primed = false;
      const unobserve = observeElementRect(instance, (rect) => {
        latestRect = rect;
        if (!primed) {
          primed = true;
          forwardedRect = rect;
        ...
        if (forwardedRect && rect.width === forwardedRect.width && rect.height === forwardedRect.height) {
          return;
        }
        this.resizing = true;
  ...
  configure(count, options) {
    ...
    if (this.virtualizer === null) {
      this.virtualizer = new Virtualizer({
        ...
        scrollPaddingStart: options.scrollPaddingStart ?? 0,
        scrollPaddingEnd: options.scrollPaddingEnd ?? 0,
        scrollToFn: elementScroll,
        observeElementRect: this.observeRectDebounced,
        observeElementOffset,
        rangeExtractor: this.extractRange,
        // Scroll updates stay immediate (smooth scrolling); resize updates are coalesced by the debounced rect
        // observer above, so skip the per-tick re-render while a resize is in flight.
        onChange: () => {
          if (this.resizing) return;
          queueMicrotask(() => {
            if (!this.resizing) this.host.requestUpdate();
          });
        }
      });
  ...
  get scrollMargin() {
    return this.virtualizer?.options.scrollMargin ?? 0;
  }
  ...
  scrollToIndex(index, align = "auto") {
    const options = typeof align === "string" ? { align } : align;
