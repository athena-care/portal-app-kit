...
async function loadAxe() {
  ...
  await import("../../chunks/axe.min.EXVZDI66.js");
  if (!window.axe) {
    throw new Error(
...
// _bundle_/src/components/data-grid/data-grid-test-helpers.ts
var people = [
...
var columns = [
  ...
  { field: "age", label: "Age", align: "end", sortable: true }
];
async function setup(grid, opts = {}) {
  grid.columns = opts.columns ?? columns;
  grid.data = opts.data ?? people;
  await grid.updateComplete;
  await aTimeout(0);
  await grid.updateComplete;
}
function tallData(n2) {
  return Array.from({ length: n2 }, (_, i2) => ({ id: i2, name: `Row ${i2}`, role: `r${i2 % 3}`, age: i2 }));
}
async function openFilterPanel(el, columnId) {
  const trigger = columnId ? el.shadowRoot.querySelector(`#filter-trigger-${columnId}`) : el.shadowRoot.querySelector(".filter-trigger");
  const popover = trigger.nextElementSibling;
  const shown = oneEvent(popover, "wa-after-show");
  trigger.click();
  await shown;
  await el.updateComplete;
  return popover;
}
function activeCell(el) {
  return el.shadowRoot.querySelector(
    '[tabindex="0"][role="gridcell"], [tabindex="0"][role="columnheader"]'
  );
}
async function waitForActiveCell(el, tries = 30) {
  for (let i2 = 0; i2 < tries; i2++) {
    const cell = activeCell(el);
    if (cell) return cell;
    await aTimeout(16);
    await el.updateComplete;
  }
  throw new Error("active cell never appeared");
}
function pointer(target, type, clientX, clientY = 10) {
