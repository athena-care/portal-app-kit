...
var WaDatePicker = class extends WebAwesomeElement {
  ...
  renderMonth(visibleMonth, isDisabled, locale, showLabel) {
    ...
    return x`
      <div part="month" data-first-day="${firstDay}" data-weekend-days="${[...weekend.values()].join(" ")}">
        ${showLabel ? x`<div part="month-label">${monthLabelFmt.format(visibleMonth)}</div>` : E}
        <table part="grid" role="grid" aria-label=${monthLabel} aria-readonly=${this.readonly ? "true" : "false"}>
          <thead>
            <tr part="weekdays">
              ${this.withWeekNumbers ? x`<th part="weeknumbers" scope="col" aria-label="Week"></th>` : E}
              ${weekdayCells.map((w) => x`<th part="weekday" scope="col" aria-label=${w.long}>${w.label}</th>`)}
            </tr>
          </thead>
          <tbody>
  ...
  renderWeek(row, visibleMonth, isDisabled, weekend, locale) {
    ...
    return x`
      <tr role="row">
        ${weekNumber !== null ? x`<th part="weeknumber" scope="row">${weekNumber}</th>` : E}
  ...
  renderDay(date, visibleMonth, isDisabled, weekend, locale) {
    ...
    if (renderedOutside) {
      return x`<td role="gridcell" aria-hidden="true"><span part="day-placeholder"></span></td>`;
    }
    const dayParts = ["day"];
    ...
    return x`
      <td
        ...
        data-in-range=${o(tdAttrs["data-in-range"])}
        data-range-start=${o(tdAttrs["data-range-start"])}
        data-range-end=${o(tdAttrs["data-range-end"])}
      >
        <button
          type="button"
          part=${dayParts.join(" ")}
          ...
          @keydown=${(e2) => this.handleDayKeydown(e2, date, disabled)}
        >
          <span part="day-label">
  ...
  renderDayFallback(date) {
    if (this.dayContent) {
      ...
      if (result != null) {
        return typeof result === "string" ? o2(result) : result;
      }
    }
    return date.getDate();
  }
      ...
      return x`
        <div role="gridcell" part="view-cell" aria-selected=${isSelected ? "true" : "false"}>
          <button
            ...
            @keydown=${(e2) => this.handleMonthKeydown(e2, year, i, monthDisabled)}
          >
  ...
  renderViewRows(count, renderItem) {
    ...
    for (let start = 0; start < count; start += columns) {
      ...
      rows.push(x`<div part="view-row" role="row">${cells}</div>`);
    }
    return rows;
  }
      ...
      return x`
        <div role="gridcell" part="view-cell" aria-selected=${isSelected ? "true" : "false"}>
          <button
            ...
            @keydown=${(e2) => this.handleYearKeydown(e2, year, yearDisabled)}
          >
            ${year}
          </button>
        </div>
...
__decorateClass([
  n({ attribute: "focused-date", reflect: true })
...
__decorateClass([
  n({ attribute: "page-by", reflect: true })
], WaDatePicker.prototype, "pageBy", 2);
__decorateClass([
  n({ attribute: "first-day-of-week", reflect: true })
], WaDatePicker.prototype, "firstDayOfWeek", 2);
__decorateClass([
  n({ attribute: "with-outside-days", type: Boolean, reflect: true })
], WaDatePicker.prototype, "withOutsideDays", 2);
__decorateClass([
  n({ attribute: "with-week-numbers", type: Boolean, reflect: true })
], WaDatePicker.prototype, "withWeekNumbers", 2);
__decorateClass([
  n({ attribute: "weekday-format", reflect: true })
...
__decorateClass([
  n({ attribute: "disabled-dates" })
], WaDatePicker.prototype, "disabledDates", 1);
__decorateClass([
  n({ attribute: "disabled-days-of-week" })
], WaDatePicker.prototype, "disabledDaysOfWeek", 2);
__decorateClass([
  n({ attribute: "disable-past", type: Boolean, reflect: true })
], WaDatePicker.prototype, "disablePast", 2);
__decorateClass([
  n({ attribute: "disable-future", type: Boolean, reflect: true })
], WaDatePicker.prototype, "disableFuture", 2);
__decorateClass([
  n({ attribute: "min-range", type: Number, reflect: true })
], WaDatePicker.prototype, "minRange", 2);
__decorateClass([
  n({ attribute: "max-range", type: Number, reflect: true })
